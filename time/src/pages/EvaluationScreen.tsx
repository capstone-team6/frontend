...
import CategoryEvaluationScreen from './CategoryEvaluatioScreen';

type ERouteProp = RouteProp<RootStackParamList, 'EvaluationScreen'>;
type NavigationProp = StackNavigationProp<
  RootStackParamList,
  'CategoryEvaluationScreen'
>;
interface prop {
  route: ERouteProp;
}
...
const EvaluationScreen: React.FC<prop> = ({route}) => {
  ...
  const BoardCategory: {[key: string]: number} = {
    TALENT: 1,
    EXERCISE: 2,
    ERRANDS: 3,
    FREE: 4,
  ...
  const handleCategoryPress = (category: string) => {
    const evaluationNum = BoardCategory[category];
    navigation.navigate('CategoryEvaluationScreen', {evaluationNum, userId});
  };
